...
async def _aplicar_novo_avatar(interaction: discord.Interaction, image_bytes: bytes) -> bool:
    ...
    if not pode_trocar:
        await interaction.followup.send(
            f"⏱️ Calma! Você já trocou a foto recentemente. Tenta de novo em **{minutos_restantes} min**.",
            ephemeral=True
        )
        return False
    ...
    if len(image_bytes) > 10 * 1024 * 1024:
        await interaction.followup.send(
            "❌ Imagem muito grande (máx 10MB pro Discord aceitar).", ephemeral=True
        )
        return False
    ...
    try:
        ...
        if e.status == 429:
            await interaction.followup.send(
                "❌ O Discord recusou por limite de trocas (429). Aguarda alguns minutos e tenta de novo.",
                ephemeral=True
            )
        else:
            await interaction.followup.send(f"❌ Erro do Discord ao trocar o avatar: {e}", ephemeral=True)
        return False
...
async def _aplicar_novo_banner(interaction: discord.Interaction, image_bytes: bytes) -> bool:
    ...
    if not pode_trocar:
        await interaction.followup.send(
            f"⏱️ Calma! Você já trocou o banner recentemente. Tenta de novo em **{minutos_restantes} min**.",
            ephemeral=True
        )
        return False
    ...
    try:
        ...
        if e.status == 429:
            ...
            await interaction.followup.send(
                "❌ O Discord recusou a troca de banner (esse recurso pode ainda não estar liberado pra "
                "conta de bot). O avatar continua funcionando normal — só o banner que não colou dessa vez.",
                ephemeral=True
            )
        else:
            await interaction.followup.send(f"❌ Erro do Discord ao trocar o banner: {e}", ephemeral=True)
        return False
    except Exception as e:
        await interaction.followup.send(f"❌ Erro inesperado: {e}", ephemeral=True)
        return False
...
def _embed_painel(guild: discord.Guild) -> discord.Embed:
    embed = discord.Embed(
        title="🖼️ Foto de Perfil & Banner do Bot — Este Servidor",
        description=(
            ...
            f"⏱️ Cooldown entre trocas: **{COOLDOWN_MINUTOS} minutos** (limite do próprio Discord, "
...
class ModalNovoAvatar(ui.Modal, title="🖼️ Nova Foto de Perfil"):
                ...
                async with sess.get(url) as resp:
                    if resp.status != 200:
                        return await interaction.followup.send(
                            f"❌ Não consegui baixar essa imagem (status {resp.status}). Verifica o link.",
                            ephemeral=True
                        )
                    image_bytes = await resp.read()
        except Exception as e:
            return await interaction.followup.send(f"❌ Erro ao baixar a imagem: {e}", ephemeral=True)

        if not await _aplicar_novo_avatar(interaction, image_bytes):
            return

        embed = _embed_painel(interaction.guild)
        await interaction.followup.send("✅ Foto de perfil atualizada só neste servidor!", ephemeral=True)
        try:
...
class ModalNovoBanner(ui.Modal, title="🎨 Novo Banner"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = _embed_painel(interaction.guild)
        await interaction.followup.send("✅ Banner atualizado só neste servidor!", ephemeral=True)
        try:
            await interaction.message.edit(embed=embed, view=PainelAvatar())
        except (discord.HTTPException, AttributeError):
            pass
...
class PainelAvatar(ui.View):
    ...
    @ui.button(label="Enviar Foto por Link", style=discord.ButtonStyle.blurple, emoji="🔗", custom_id="avatar_enviar", row=0)
    ...
    @ui.button(label="Restaurar Foto Padrão", style=discord.ButtonStyle.grey, emoji="↩️", custom_id="avatar_resetar", row=0)
    async def btn_resetar(self, interaction: discord.Interaction, button: ui.Button):
        pode_trocar, minutos_restantes = await _checar_cooldown(interaction.guild_id, "avatar")
        if not pode_trocar:
            return await interaction.response.send_message(
                f"⏱️ Calma! Tenta de novo em **{minutos_restantes} min**.", ephemeral=True
            )
        ...
        embed = _embed_painel(interaction.guild)
        await interaction.followup.send("✅ Avatar deste servidor restaurado pro padrão global.", ephemeral=True)
        try:
            await interaction.message.edit(embed=embed, view=PainelAvatar())
        except (discord.HTTPException, AttributeError):
            pass

    @ui.button(label="Enviar Banner por Link", style=discord.ButtonStyle.blurple, emoji="🎨", custom_id="banner_enviar", row=1)
    ...
    @ui.button(label="Restaurar Banner Padrão", style=discord.ButtonStyle.grey, emoji="↩️", custom_id="banner_resetar", row=1)
    async def btn_resetar_banner(self, interaction: discord.Interaction, button: ui.Button):
        ...
        embed = _embed_painel(interaction.guild)
        await interaction.followup.send("✅ Banner deste servidor restaurado pro padrão.", ephemeral=True)
        try:
            await interaction.message.edit(embed=embed, view=PainelAvatar())
        except (discord.HTTPException, AttributeError):
            pass
...
class AvatarManager(commands.Cog):
    ...
    async def avatarbot(self, interaction: discord.Interaction, imagem: discord.Attachment = None, banner: discord.Attachment = None):
        ...
        if imagem is not None and not (imagem.content_type or "").startswith("image/"):
            return await interaction.response.send_message(
                "❌ O anexo de foto de perfil não parece ser uma imagem. Anexa um png, jpg ou webp.", ephemeral=True
            )
        if banner is not None and not (banner.content_type or "").startswith("image/"):
            return await interaction.response.send_message(
                "❌ O anexo de banner não parece ser uma imagem. Anexa um png, jpg ou webp.", ephemeral=True
            )
