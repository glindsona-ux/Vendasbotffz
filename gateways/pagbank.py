"""
gateways/pagbank.py — ENCAIXE PRONTO, ainda não implementado de verdade.

Por quê: eu não tenho uma conta de vendedor + API key do PagBank pra
...
Quando o admin escolhe PagBank em `/configurarloja` mas tenta gerar uma
...
class GatewayPagBank(GatewayPagamento):
    ...
    async def criar_cobranca(self, *, valor, descricao, pedido_id, notification_url) -> ResultadoCobranca:
        return ResultadoCobranca(
            ok=False,
            erro=(
                "O gateway PagBank ainda não está com a integração automática pronta "
